...
public class DiaryManager2 : MonoBehaviour {
    ...
    private IEnumerator NextDay() {
        ...
        DialogueEvent dialogue = GetComponent<DialogueEvent>();
        dialogue.dialogues[0].Condition = () => true;
        dialogue.dialogues[0].OnDialogueComplete = () => {
            StartCoroutine(NextDayOn());
        };
        dialogue.dialogues[1].Condition = () => false;
        dialogue.OnInteract();
        ...
        yield return new WaitUntil(() => fadeInBackground.alpha >= 1f);
        yield return new WaitForSeconds(2f);

        dialogue.dialogues[0].Condition = () => false;
        dialogue.dialogues[1].Condition = () => true;
        dialogue.dialogues[1].OnDialogueStart = () => {
            StartCoroutine(NextDayOff());
        };
        dialogue.dialogues[1].OnDialogueComplete = () => {
            Debug.Log("다음 날이다. 할머니한테 가서 대화를 해보자.");
        };
        dialogue.OnInteract();
    }

    private IEnumerator NextDayOn() {
        ...
        while (elapsed < 2) {
            ...
            fadeInBackground.alpha = Mathf.Clamp01(elapsed / 2);
            yield return null;
        }
        player.position = new Vector3(3.75f, 0, -0.76f); // 플레이어 위치 초기화
        player.rotation = Quaternion.Euler(0, 0, 0);
        mainCam.position = new Vector3(3.75002408f, 1.96652663f, -2.30362725f); // 카메라 위치 초기화
        mainCam.rotation = Quaternion.Euler(16.8163166f, 0, 0);
    }

    private IEnumerator NextDayOff() {
        float elapsed = 0f;
        fadeInBackground.alpha = 1f; // 처음엔 완전히 보이게

        while (elapsed < 2f) {
            elapsed += Time.deltaTime;
            fadeInBackground.alpha = Mathf.Clamp01(1f - (elapsed / 2f)); // 1 → 0으로 감소
            yield return null;
        }
        fadeInBackground.gameObject.SetActive(false);
    }
